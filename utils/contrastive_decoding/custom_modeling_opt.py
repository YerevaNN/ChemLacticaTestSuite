...
class CustomOPTLearnedPositionalEmbedding(nn.Embedding):
    """
...
class CustomOPTAttention(nn.Module):
...
class CustomOPTDecoderLayer(nn.Module):
    def __init__(self, config: OPTConfig):
        super().__init__()
        self.embed_dim = config.hidden_size

        if not getattr(config, "_flash_attn_2_enabled", False):
            self.self_attn = CustomOPTAttention(
...
class CustomOPTPreTrainedModel(PreTrainedModel):
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, (CustomOPTDecoder)):
...
class CustomOPTDecoder(CustomOPTPreTrainedModel):
    """
    ...
    def __init__(self, config: OPTConfig):
        ...
        self.embed_positions = CustomOPTLearnedPositionalEmbedding(config.max_position_embeddings, config.hidden_size)
        ...
        self.layers = nn.ModuleList([CustomOPTDecoderLayer(config) for _ in range(config.num_hidden_layers)])
...
class CustomOPTModel(CustomOPTPreTrainedModel):
    def __init__(self, config: OPTConfig):
        super().__init__(config)
...
class LinearFloat32(torch.nn.Linear):
    """
        Class that casts the logits to float32
    """
    def forward(self, _input: torch.Tensor) -> torch.Tensor:
        return super().forward(_input).to(torch.float32)


class CustomOPTForCausalLM(CustomOPTPreTrainedModel):
...
class CustomOPTForSequenceClassification(CustomOPTPreTrainedModel):
    def __init__(self, config: OPTConfig):
        super().__init__(config)
...
class OPTForQuestionAnswering(CustomOPTPreTrainedModel):
    def __init__(self, config: OPTConfig):
        super().__init__(config)
        self.model = CustomOPTModel(config)
