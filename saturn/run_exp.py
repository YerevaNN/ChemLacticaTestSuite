...
if __name__ == "__main__":
    ...
    set_seed_everywhere(args.seed, config["device"])
    set_seed(args.seed)

    oracle = SaturnDockingOracle(3000, args.target, takes_entry=True)

    config["log_dir"] = os.path.join(args.output_dir, f"results-taget-{args.target}-seed-{args.seed}.log")
