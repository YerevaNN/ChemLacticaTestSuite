...
class GenerationMixin:
    ...
    def _prepare_encoder_decoder_kwargs_for_generation(
        ...
        # print(f'get encoder, see here. {self}')
        # 1. get encoder
    ...
    def generate(
        ...
        **model_kwargs,
        ...
        if is_greedy_gen_mode:
            ...
            if model_kwargs['teacher_student']:
                student_top_k = model_kwargs.pop('student_top_k', 0)
                student_top_p = model_kwargs.pop('student_top_p', 1.0)
                student_min_prob = model_kwargs.pop('student_min_prob', 0.0)
                student_temperature = model_kwargs.pop('student_temperature', 1.0)
                logits_warper_student = self._get_logits_warper(
                    top_k=student_top_k,
                    top_p=student_top_p,
                    min_prob=student_min_prob,
        ...
        **model_kwargs,
        ...
        while True:
            ...
            if model_kwargs['teacher_student']:
                ...
                #     print('teacher past key val ')
                #     print(model_inputs['past_key_values'][0][0].shape, model_inputs['past_key_values'][1][0].shape)
                
                # print(model_inputs_student['attention_mask'].shape)
                # DEBUGv1
                # print(model_inputs_student.keys())
                # print('input_ids', 'encoder_outputs', 'past_key_values', 'decoder_input_ids',)
                # print(
                #        model_inputs_student['decoder_input_ids'].shape, 
                #       )
                # if first_marker:
            ...
            if model_kwargs['use_switch']:
                ...
                temp_teacher = torch.topk(switch_criteria_teacher, dim=-1, k=2)
                temp_student = torch.topk(switch_criteria_student, dim=-1, k=2)
                print(temp_teacher.values, temp_teacher.indices)
                print(temp_student.values, temp_student.indices)
                print(input_ids)
                print(10/0)
            ...
            if False and model_kwargs['teacher_student']:
                ...
                # mask_cap = (next_token_logits_student <  -50) 
                # mask_cap2 =  (next_token_logits_student == float('inf')) 
                # next_token_logits_student[mask_cap] = -50
                # next_token_logits_student[mask_cap2] = -50

                next_token_scores = post_process_easy(next_token_scores, next_token_logits_student, model_kwargs)
                # print((next_token_scores > float('-inf')).sum())
                next_token_scores = next_token_scores.view(batch_size, num_beams * vocab_size)
                next_token_scores, next_tokens = torch.topk(
                    next_token_scores, 2 * num_beams, dim=1, largest=True, sorted=True
                )
                next_token_scores_keep = next_token_scores_keep.view(batch_size, num_beams * vocab_size)
                next_token_scores2, next_tokens2 = torch.topk(
                    next_token_scores_keep, 2 * num_beams, dim=1, largest=True, sorted=True
                )
                print(next_token_scores2.exp(), next_token_scores2.mean())
                next_token_scores_keep = torch.gather(next_token_scores_keep, -1, next_tokens)
                print(next_token_scores_keep.exp(), next_token_scores_keep.mean())
                ...
                if model_kwargs['use_cap_student']: 
                    ...
                    cap_student = cap_student.unsqueeze(1).expand(-1, next_token_logits_student.size(1))
                    mask_cap = next_token_logits_student < cap_student
                    next_token_logits_student[mask_cap] = cap_student[mask_cap]
                if model_kwargs['use_switch']:
                    next_token_logits_student[batch_switch] = 0. 
                vocab_size = next_token_scores.shape[-1]
                next_token_scores = next_token_scores.view(batch_size, num_beams * vocab_size)
                next_token_scores_keep = next_token_scores_keep.view(batch_size, num_beams * vocab_size)
                ...
                # print(model_kwargs['tokenizer'].batch_decode(next_tokens.view(-1, 1)))
                # print(model_kwargs['tokenizer']('.'))
                # print(next_tokens)
                if True: 
                    next_indices, next_tokens, next_token_scores = post_process_reweight(input_ids, next_indices, next_tokens,
                                                                                        next_token_scores, student_scores, model_kwargs, self)
                else: #jason's version
                    next_indices, next_tokens, next_token_scores = post_process_reweight_v2(input_ids, next_indices, next_tokens,
                                                                                        next_token_scores, student_scores, model_kwargs, self, next_token_scores_keep)                                                                   
                # LISA 
                # print(next_token_scores) 
        ...
        **model_kwargs,
        ...
        while True:
            ...
            if model_kwargs['teacher_student']:
                ...
                model_inputs_student = self.prepare_inputs_for_generation(input_ids, **model_kwargs_student)
                # print(model_inputs_student.keys(), model_inputs_student['input_ids'].shape, )
                outputs_student = model_kwargs['student_lm'](
                    **model_inputs_student,
                    return_dict=True,
                    output_attentions=output_attentions,
            ...
            # next_token_logits = self.post_hoc_logits_adjust_teacher_student(next_token_logits, logits_student=next_token_logits_student, 
            #                                                     st_coef=model_kwargs['st_coef'])
            # next_token_scores = nn.functional.log_softmax(next_token_logits, dim=-1) 
            # next_token_scores = self.post_hoc_mi_adjust_teacher_student(next_token_logits, logits_student=next_token_logits_student, 
            #                                                     st_coef=model_kwargs['st_coef'])
            next_token_scores = nn.functional.log_softmax(
                next_token_logits  + next_token_logits_student, dim=-1
            )  # (batch_size * num_beams, vocab_size)

            # print('take the diff')
            # next_token_scores = next_token_scores - next_token_logits_student
            ...
            # next_tokens_keep = next_tokens.clone()
            # student_scores = torch.gather(next_token_logits_student, -1, next_tokens_keep)
            # next_token_scores_keep = torch.gather(next_token_scores_keep, -1, next_tokens_keep)
